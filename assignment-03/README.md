...
I trained this model for Assignment 3 of Fundamentals of Agentic AI. It's the course's word-token nanoGPT notebook, a two-block, four-head transformer with 64-number embeddings and a 48-token context, run on my laptop's CPU. The assignment asks for two experiments on the same 48 fixed language tests. In the first I trained on the course's classroom corpus for 3,000 steps at a learning rate of 0.001, and the trained model got 20 of the 48 tests right, with 24 of the 48 unscorable because their words were not in the vocabulary. Its weights came out identical to the course's published reference model, which I checked by hash. In the second I added 2,215 teaching passages of my own for four of the eight extension skills, grammar, opposites, negation, and reference, and trained a fresh model with the same two settings. That made 12 more tests scorable and the trained model got 35 of 48, with the one miss in reference. My teaching files went through two stricter versions before the instructor clarified in class on 2026-09-22 that the one thing that must stay out of the corpus is the eval questions themselves, so the files now drop only lines that contain a test prompt, which I explain under Keeping the tests out of training. I then ran an optional third experiment that doubled the steps to 6,000 and changed nothing else I set. It got all 12 of the extension tests in the four skills I taught but lost 1 of the 8 new phrasings of the starter patterns, for 35 of 48 again, and I read that with two caveats, that the learning rate's decay stretched to match the longer run and that each budget was trained once on one seed. The notebook in this folder is the second experiment, executed start to finish with every output saved, and the starter and optional runs are archived under `results/` with their own executed notebooks.
...
I kept 3,000 steps for both required runs because it's the course's starting budget and because I wanted the corpus to be the only thing that changed between them. 3,000 steps draws 96,000 passages in batches of 32, which is about 23 passes over the starter run's 4,132 training passages and about 16 over the extended run's 6,126, and for text built from repeated sentence frames I thought that was plenty. The optional third run changed only this setting, to 6,000, for a reason that comes out of the second run's results.
...
Before the corpus-extension run, which I ran after the instructor's clarification, I expected coverage to stay at 36 scorable cases and the 16 starter patterns and 8 new phrasings to hold at 16 and 8, as in both earlier versions. I expected grammar, opposites, and reference to stay at 3 of 3, with opposites back at high confidence now that the pair statements were in the files, and negation to rise to 2 of 3, with the box and the door right now that the red-to-blue and open-to-closed corrections were back, and ava still getting a favorite food, for around 35 of 48. The prediction cell also says those corrections came back on doors, which was wrong, since the open-to-closed ones landed on a gate, a wall, and a coat, while one red-to-blue correction did land on a box.

Before the optional third run I expected 36 of 48, with the pencil story flipping to finn, since more training had fixed name mix-ups like that one in both earlier versions, and every other extension case staying right. I expected the 8 new phrasings to hold at 8 of 8 with lower probabilities than at 3,000 steps, since both earlier 6,000-step runs pushed most of those down and one of them lost three.
...
The 24 extension tests cover eight skills the classroom sentences never touch, and in the starter run every one of them was unscorable, since the runner refuses to score a case whose prompt or answer choices include a word outside the vocabulary. I chose four skills to teach, grammar, opposites, negation, and reference, because they are teachable with short sentences and because they sit at different distances from what the starter corpus already does. Grammar is agreement, a local pattern between neighboring words, which is the kind of thing a next-word model picks up first. Opposites is a word association like the starter's own domain associations. My file states all 24 pairs of opposites outright, including the three pairs the tests ask about, as in `the opposite of cold is hot .`, and uses them in contrasts like `the cup was cold but the pillow was hot .`, and the only lines missing are the three that would contain a test prompt, like `the opposite of hot is cold .`. Negation and reference need the model to reach back across a sentence boundary and copy a specific earlier word, which is the thing attention is for, and reference also needs it to tell the two people in a story apart. I left sequence, spatial relations, everyday knowledge, and categories alone, so their 12 cases stay unscorable in every run and count as zero in the all-case rate.
...
| [corpus/opposites.txt](corpus/opposites.txt) | 333 | 24 pairs of opposites in four short frames and four sentence frames, each pair written in both directions, less the three short lines that would contain a test prompt | `the opposite of cold is hot .` and `the cup was cold but the pillow was hot .` |
...
| [corpus/reference.txt](corpus/reference.txt) | 528 | Two-sentence stories in which one person gives, lends, hands, sends, calls, phones, or writes to another, and the second sentence says who thanked, answered, received, or gave | `eli gave mia a gift.mia thanked eli.` |

The stories in the last two files are written with no space after the periods inside them, and that is deliberate. The notebook splits imported text into passages wherever a period is followed by whitespace, so a three-sentence story written normally would become three one-sentence passages and the model would never see a period followed by more words, let alone learn to copy a word from before one. Written this way each story stays one passage, and the tokenizer still separates the periods, so the model sees `the bike was not brown . it was green . the bike was green .` as one sequence of 15 tokens plus the start and end tokens, the same shape as the test prompts. `corpus_manifest.json` shows all 542 negation lines and 528 reference lines arriving as exactly that many passages.

No test prompt appears in the material. The notebook withholds classroom sentences containing a reserved prefix, rejects any imported file containing an exact test prompt, and checks every final passage again, and my generator drops any line that contains a test prompt before it writes the files and rereads them afterward to check. That is the only rule I apply, and everything else about the tests, their words, pairs, and sentence frames, is taught on purpose, for the reason given under Keeping the tests out of training.
...
The samples, from [results/expanded/samples/](results/expanded/samples/), show the one visible change I'd point to in this run. At step 1,500 the first line is a reference story that loses track of both the object and the name, `luca handed a card to rosa . the person who received the pencil was theo .`, and at step 3,000 the same seed produces `luca handed a key to rosa . rosa thanked luca .`, with the names the right way around. These are draws at temperature 0.8, so a single line says what the model happened to produce and only the probabilities say what it prefers, which is what the probes below are for.
...
step 1500
luca handed a card to rosa . the person who received the pencil was theo .
...
step 3000
luca handed a key to rosa . rosa thanked luca .
a review of order helped us understand the different client .
a review of price helped us understand the important item .
today the market focused on price and the important product .
```

On the 48 tests the untrained model got 7 of 48 with 36 scorable, and the trained model got 35 of 48, all 16 starter patterns, all 8 new phrasings, and 11 of the 12 newly scorable extension cases. Grammar, opposites, and negation were 3 of 3 each and reference was 2 of 3. I predicted about 35 and got 35, but not in the way I predicted, since I expected negation at 2 of 3 and reference at 3 of 3 and got the reverse. Opposites came back to high confidence, cold at 0.464 against warm at 0.014 for the hot case, full at 0.202, and quiet at 0.406, close to my first version's 0.451, 0.183, and 0.399 and well above the 0.093, 0.037, and 0.179 of the version that held the pair statements back. The new phrasings went from 4 of 8 in the starter run to 8 of 8. Two things changed between those runs at once, the training data and, because the vocabulary is a different size, the network's random starting weights, so I can't say from these two runs alone that the new material caused that gain. One plausible mechanism is that the classroom sentences only ever put yesterday at the end of a sentence while my grammar file puts it at the start in 36 of the 200 lines that use it, and two of the four phrasings the starter model missed begin with yesterday, but I haven't tested that.

The one miss is the concrete failure in this run. For `ella gave finn a pencil . the person who received the pencil was` it chose ella, the giver, at 0.190 over finn at 0.081. The three negation passes are not all what they look like. The ava case chose milk at 0.115, and the probes below show this model answered milk for what nora bought whichever way the story went, so that pass matches a favorite word. The box and the door look more like real copying, since the probes show colors, doors, and gates following the story, though the box has a near-copy in my negation file, `this box is not red.it is blue.this box is blue.`, and the door has none, so the door is the cleaner case.

### The optional 6,000-step run

I first proposed doubling the training steps as my next experiment after my first version's 3,000-step model missed two negation cases and, in the probes, answered several flipped stories with a favorite word or name, and I reran it on each version of the corpus, keeping the corpus, the learning rate, the seed, and the split exactly the same as the corpus-extension run. I chose that budget after seeing 3,000-step evals and probes, so its scores are not independent evidence that more training helps. It's archived at [results/expanded-6000-steps/](results/expanded-6000-steps/) and it is extra evidence, not one of the two required experiments.
...
The panels are the same 20 and 20 documents as the corpus-extension run, so these are comparable to that run's table, with the caveat that the cosine decay was spread over twice the steps, which is why the step-3,000 row here is not the same as the step-3,000 row above. The training panel kept falling a little after step 3,000 while the validation panel rose slightly. The trained model got 35 of 48. Every one of the 12 extension cases was right, most of them near certainty, with finn at 0.837 for the pencil story, milk at 0.446 for ava, and closed at 0.954 for the door, but 1 of the 8 new phrasings of the starter patterns went wrong, so the total stayed at 35. My prediction of 36 was one off. The pencil story flipped to finn as I expected, and the new phrasings dropped as I expected, except that one dropped far enough to flip.

The phrasing it lost is `yesterday the school discussed the educator and the`, where all four choice words fell below 0.0001 and risk edged out student. All eight new-phrasing answer probabilities fell between 3,000 and 6,000 steps, five of them below 0.004, and both earlier versions of my corpus lowered six of the eight at 6,000 steps, one keeping all eight right and one losing three, so which ones flip looks like chance at these margins, and the drop itself is the consistent finding. I read it as the longer run fitting the model more tightly to the exact classroom frames, which is the brief's warning that held-out sentences share templates with training, but it's one run of each on one seed, and I did not run a version that separates the extra steps from the stretched schedule.
...
| Expanded corpus | Trained, 3,000 steps | 35 | 36 | 97.2% | 16 of 16 | 8 of 8 | 11 of 24, 12 scorable | [results/expanded/language_evals/final/](results/expanded/language_evals/final/) |
| Expanded corpus, optional run | Trained, 6,000 steps | 35 | 36 | 97.2% | 16 of 16 | 7 of 8 | 12 of 24, 12 scorable | [results/expanded-6000-steps/language_evals/final/](results/expanded-6000-steps/language_evals/final/) |
...
| New wording | 8 | 3 of 8 | 4 of 8 | 4 of 8 | 8 of 8 | 7 of 8 |
| Grammar | 3 | none scorable | none scorable | 0 of 3 | 3 of 3 | 3 of 3 |
| Opposites | 3 | none scorable | none scorable | 0 of 3 | 3 of 3 | 3 of 3 |
| Negation | 3 | none scorable | none scorable | 0 of 3 | 3 of 3 | 3 of 3 |
| Reference | 3 | none scorable | none scorable | 0 of 3 | 2 of 3 | 3 of 3 |
...
Within the scorable cases, the gains come from training on patterns, and the probes below say how much of that is the pattern itself. The starter patterns went from 6 to 16 in the first experiment on the classroom sentences alone, and the 12 new extension cases went from 0 to 11 at 3,000 steps and to 12 at 6,000 steps on my files, while the optional run gave back 1 of the 8 new phrasings. The eval guide's warning that a bigger vocabulary can move the accuracy among scorable cases in either direction applies here in a mild way, since 35 of 36 is a different denominator from 20 of 24, which is why I've kept both numbers in the table.
...
| --- | --- | --- | --- | --- | --- | --- |
| lang_25, grammar | one bird | is | is, right | is 0.512, are 0.000, were 0.000, am 0.000 | was busy yesterday . | is, right |
| lang_26, grammar | the dogs | are | are, right | is 0.000, am 0.000, was 0.000, are 0.611 | are clean . | are, right |
| lang_27, grammar | yesterday she | walked | walked, right | walking 0.001, walk 0.001, walked 0.027, walks 0.011 | fast at the park . | walked, right |
| lang_28, opposites | the opposite of hot is | cold | cold, right | fast 0.000, cold 0.464, warm 0.014, heavy 0.002 | cold . | cold, right |
| lang_29, opposites | the opposite of empty is | full | full, right | full 0.202, quiet 0.000, early 0.000, soft 0.001 | yellow . | full, right |
| lang_30, opposites | the opposite of noisy is | quiet | quiet, right | loud 0.043, round 0.002, late 0.013, quiet 0.406 | rest . | quiet, right |
| lang_31, negation | the box is not red . it is blue . the box is | blue | blue, right | green 0.056, yellow 0.190, blue 0.291, red 0.040 | black . | blue, right |
| lang_32, negation | ava did not buy tea . she bought milk . ava bought | milk | milk, right | rice 0.026, milk 0.115, tea 0.024, bread 0.016 | honey . | milk, right |
| lang_33, negation | the door is not open . it is closed . the door is | closed | closed, right | closed 0.431, open 0.255, wide 0.000, missing 0.000 | closed . | closed, right |
| lang_34, reference | maya lent a book to leo . leo thanked | maya | maya, right | leo 0.092, nora 0.000, omar 0.000, maya 0.624 | maya . | maya, right |
| lang_35, reference | ella gave finn a pencil . the person who received the pencil was | finn | ella, wrong | sara 0.002, noah 0.055, finn 0.081, ella 0.190 | ava . | finn, right |
| lang_36, reference | omar called nina . nina answered the call from | omar | omar, right | emma 0.000, omar 0.801, nina 0.002, luca 0.002 | omar . | omar, right |
| lang_37, sequence | first wash the cup . then dry it . the last action is | dry | unscorable | unknown words action, fill, first, wash | late . | unscorable |
| lang_38, sequence | lunch happens after breakfast . the earlier meal is | breakfast | unscorable | unknown words breakfast, dinner, earlier, happens, lunch, meal, supper | small . | unscorable |
| lang_39, sequence | the train arrived before the bus . the vehicle that arrived later was the | bus | unscorable | unknown words arrived, before, later, that, vehicle | wall is not | unscorable |
| lang_40, spatial relations | the book is inside the bag . the bag contains the | book | unscorable | unknown words contains, desk, inside, shelf | bag is not low . | unscorable |
| lang_41, spatial relations | the lamp is above the desk . the desk is | below | unscorable | unknown words above, below, beside, desk, inside | not pink . | unscorable |
| lang_42, spatial relations | the ball is left of the box . the box is to the | right | unscorable | unknown words left, north, right, south | ball . | unscorable |
| lang_43, everyday knowledge | water freezes into | ice | unscorable | unknown words freezes, ice, into, sand, steam, wood | the bag is the opposite opposite of patient . | unscorable |
| lang_44, everyday knowledge | a person uses an umbrella to stay | dry | unscorable | unknown words an, stay, umbrella, uses | the person who received the ball was | unscorable |
| lang_45, everyday knowledge | to see in a dark room we turn on a | light | unscorable | unknown words see, shoe, spoon, turn | discussion of low . | unscorable |
| lang_46, categories and analogies | a robin is a bird . a salmon is a | fish | unscorable | unknown words robin, salmon, tool, tree | it focused on harvest . | unscorable |
| lang_47, categories and analogies | a puppy grows into a dog . a kitten grows into a | cat | unscorable | unknown words grows, into, kitten, puppy | bag is purchase . | unscorable |
| lang_48, categories and analogies | a carrot is a vegetable . an apple is a | fruit | unscorable | unknown words an, carrot, fabric, metal, vegetable, vehicle | review of course focused on . | unscorable |

The continuations for the unscorable cases are worth a look on their own. The model receives those prompts with their unknown words replaced by the `<UNK>` token, and what it writes back is a fragment of whatever frame the known words suggest, `bag is not low .` after the bag prompt, which is the start of one of my negation corrections, and `the person who received the ball was` after the umbrella prompt, which is a piece of my reference frame with nothing to anchor it.

### What the scores do and do not show

A right answer on a four-choice test is a narrow thing, so after training I wrote extra prompts of my own, seventeen in [scripts/probe_model.py](scripts/probe_model.py), twelve at first and five more after a review pass over this README, to ask what the model had actually learned. They are not part of the suite and they never trained anything, and I wrote them after seeing my first version's results, so I treat them as diagnostics. With only the eval-question rule in place, one of their stories, `the cup is not black . it is white . the cup is`, now appears word for word in my negation file, and two of the three grammar prompts, `one cat` and `the cats`, start lines in my grammar file that go on with the answer, so those three passes show recall, and only the other fourteen can show transfer. The results are [results/expanded/probes.json](results/expanded/probes.json) for the 3,000-step model and [results/expanded-6000-steps/probes.json](results/expanded-6000-steps/probes.json) for the 6,000-step one.

At 3,000 steps colors, doors, and gates followed the story. The flipped cups got black at 0.479 and white at 0.457, and the door and gate stories got open at 0.434 when the story said open and closed at 0.450 when it said closed. Foods and names did not follow the story. The model answered milk for what nora bought whether the story said rice or bread, at 0.114 both times. Given `nora gave omar a coin`, it answered omar whether the question was who gave, at 0.559, or who received, at 0.591, and it answered nora for both orders of a nora and sara pencil story, at 0.167 and 0.561. After thanked it did pick the first name in the one story I flipped, leo at 0.812. In all, 13 of the 17 probes passed, and the passes were the three grammar probes, the two color copies, the three door and gate stories, the object copy, the thanked story, and three that simply picking the second name in the story would also get.

At 6,000 steps all 17 probes pass and the answers move with the story at high confidence. The flipped cups get black at 0.946 and white at 0.959, the door and gate stories get open at 0.951 and 0.941 when the story said open and closed at 0.957 when it said closed, the giver and receiver probes get nora at 0.894 and omar at 0.960, and the two orders of the pencil story get sara at 0.969 and nora at 0.968. The weakest pair is the food stories, bread at 0.036 and rice at 0.166, which are right but far from certain. That is the behavior the negation and reference tests are meant to measure, and it appeared in the one run with twice the steps and the stretched schedule. So I'd read the corpus-extension run's 35 of 48 as vocabulary coverage plus a mix of real patterns and shortcuts, including the ava pass, and the optional run's 35 of 48 as the same coverage with the shortcuts replaced by the patterns and one new phrasing lost, at least for the frames I wrote.
...
The 48 prompts, their choices, the answer key, and every eval output live in `evals/` and `results/`, and only `corpus/` is a training input. The notebook rejects a corpus folder that includes `evals/` or the project root, withholds every generated classroom sentence containing one of the 16 reserved prefixes before the split and before the vocabulary is built, rejects any imported file containing an exact test prompt, and checks every final passage once more. [results/starter/eval_separation.json](results/starter/eval_separation.json) and [results/expanded/eval_separation.json](results/expanded/eval_separation.json) record 160 withheld passages covering all 16 reserved cases in each run, along with the suite's hash. My generator applies the same rule to the extension cases, dropping any line that contains a test prompt, and the chat transcripts were written after training and never entered a corpus.

My teaching files went through three versions in two days. The first, committed on 2026-09-21, also refused the first sentence of every test story, never put the three test couples together, and never put a test subject with its test items, so ava never appeared with tea or milk. On 2026-09-22, worried about the brief's leakage penalty, I added a filter that removed 58 more lines, like `the opposite of cold is hot .` and `eli handed a pencil to finn.the person who received the pencil was finn.`. Later that day the instructor clarified in class that the eval questions themselves are what must stay out of the corpus, that including them is cheating, and that guardrails stricter than that should come out so relevant material stays in, so I removed every rule except that one. The files now teach the tests' words, pairs, and sentence frames on purpose, and a line that would contain a test prompt, like `the opposite of hot is cold .`, is never written. Some lines now sit one or two words from a test with its answer. The reference file has one story for every ordered pair of names, so two stories keep a test couple in the test's roles with a different object or verb, `ella gave finn a toy.the person who received the toy was finn.` and `maya sent a key to leo.leo thanked maya.`, and the negation file has `this box is not red.it is blue.this box is blue.`. None of them is an eval question, which is the line the instructor drew. The course's own classroom corpus works the same way, since it withholds each reserved test prefix while keeping five to eight sentences one word away from it with its answer, like `the report about the dentist explains the patient in detail .` beside the surgeon test.

Both earlier versions are in the repository's history, [the first at commit 01b6c8b](https://github.com/IsaacAVazquez/Fundamentals-of-Agentic-AI/blob/01b6c8b/assignment-03/README.md) and [the second at commit 325a731](https://github.com/IsaacAVazquez/Fundamentals-of-Agentic-AI/blob/325a731/assignment-03/README.md), each with its own files and results. At 3,000 steps the three versions scored 34, 34, and 35 of 48, and at 6,000 steps 36, 33, and 35. The pair statements are the clearest signal, with cold at 0.451, 0.093, and 0.464 for the hot case across the three. The negation cases are mixed, since the door went right, wrong, and right as the open-to-closed corrections, all on other objects, came, went, and came back, but the box went wrong, right, and right while the red-to-blue corrections did the same. The three versions start from identical weights, since the vocabulary is the same 424 words each time and the untrained models hash to the same value, but each change to the corpus also changed the split and the order of the training batches, so this comparison is suggestive and not a clean test of what any one rule did.
...
Model: the team discussed the car was
Unknown words: ?, capital, france, what
```

The first three replies are what the corpus taught. The fourth is a negation failure. The model drew white, the color the story rejected, but for this exact prompt the probes in [results/expanded/probes.json](results/expanded/probes.json) put black first at 0.479 and white at 0.085, so the reply was an unlikely draw at temperature 0.8. The fifth is the limitation the assignment asks to show. The question's words are mostly outside the vocabulary, the model receives them as unknown tokens, and it replies with `the team discussed the car was`, pieces of a classroom frame and a negation story with no meaning together, because continuing text is the only thing it does and it has no idea of a question. The notebook's own chat cell, section 10, produced one more turn from the same model during Run All, `the customer` followed by `returned the item after checking the price .`, saved in [results/expanded/chat_transcript.json](results/expanded/chat_transcript.json).

I also ran a shorter session against the optional 6,000-step model, in [results/chat/chat_transcript_6000_steps.json](results/chat/chat_transcript_6000_steps.json), with its log beside it and [a drawing of that log](results/chat/chat_session_6000_steps.png) made by [scripts/render_session.py](scripts/render_session.py), since only the main session has a screenshot. It answered the same cup story with black, answered `nora gave omar a coin . the person who gave the coin was` with nora, continued `the customer` with `reviewed the merchandise after checking the price .`, and, given a sentence that mixes words from three of the classroom domains, `the surgeon and the mango walked to the bank`, replied with a single period, one draw that ends the sentence where it is. Both sessions used the interface's default temperature of 0.8 and 24-token limit, and neither prompt was ever longer than the context.

## One limitation and the next experiment

The limitation I'd point to is that a passing score on these tests can come from a shortcut. At 3,000 steps the model passed the ava test with milk, and the probes showed it answered milk for what nora bought whichever way the story went, so that pass came from a favorite word and not from reading the story. The names are the other face of it, since in the gave and received stories it often picked the second name whatever the question asked, and the pencil test, where the right answer was the second name but the model picked the first, shows that shortcut isn't even consistent. The multiple-choice score can't tell those apart from the real pattern, and neither can a fluent sample, which is why I'd trust the flipped-story probes more than either. In the one run with twice the steps and the stretched schedule, those shortcuts went away for the seventeen probes I wrote while one new phrasing flipped, and I haven't tested frames I didn't write.
