...
I trained this model for Assignment 3 of Fundamentals of Agentic AI. It's the course's word-token nanoGPT notebook, a two-block, four-head transformer with 64-number embeddings and a 48-token context, run on my laptop's CPU. The assignment asks for two experiments on the same 48 fixed language tests. In the first I trained on the course's classroom corpus for 3,000 steps at a learning rate of 0.001, and the trained model got 20 of the 48 tests right, with 24 of the 48 unscorable because their words were not in the vocabulary. Its weights came out identical to the course's published reference model, which I checked by hash. In the second I added 2,150 teaching passages of my own for four of the eight extension skills, grammar, opposites, negation, and reference, and trained a fresh model with the same two settings. That made 12 more tests scorable and the trained model got 34 of 48, with the two misses both in negation. The first version of my teaching files had 58 more lines that sat too close to the tests, like `the opposite of cold is hot .`, and I removed them and reran before submitting, which I explain under Keeping the tests out of training. I then ran an optional third experiment that doubled the steps to 6,000 and changed nothing else I set. It got all 12 of the extension tests in the four skills I taught, including both negation misses, but lost 3 of the 8 new phrasings of the starter patterns, for 33 of 48, and I read both changes with two caveats, that the learning rate's decay stretched to match the longer run and that each budget was trained once on one seed. The notebook in this folder is the second experiment, executed start to finish with every output saved, and the starter and optional runs are archived under `results/` with their own executed notebooks.
...
| Chat code, launch instructions, a screenshot and a transcript of at least three prompts, the run used | Chat with the trained model |
...
To run it again locally, clone the repository and set up Python 3.13 in this folder. I used `scripts/setup.sh`, which builds `.venv` with uv from Homebrew's Python 3.13 and installs `requirements.txt` plus NumPy, Pillow, pip, and the Jupyter pieces a scripted run needs. NumPy is there because the course's `run_evals.py` hashes model weights through NumPy and the PyTorch wheel does not install it, and Pillow is only for `scripts/render_session.py`. The course's own instructions use a plain virtual environment and pip instead, which also works as long as NumPy is added, and on 2026-09-21, with the first version of my files, I checked that `python3.13 -m venv .venv` followed by `.venv/bin/pip install -r requirements.txt numpy` ran the notebook, the evals, and the chat with the same results as the uv environment. Either way the notebook's setup cell installs the PDF reader if it's missing.
...
I added no PDFs, so the extraction checks reduce to four UTF-8 text files. Section 3 printed each file's passage count and a preview and reported no warnings, and `corpus_manifest.json` shows every file's passage count equal to its unique passage count and to its line count, 812, 312, 523, and 503, so each three-sentence story arrived as a single passage, which matters for the reason given under The teaching material I added.
...
I kept 3,000 steps for both required runs because it's the course's starting budget and because I wanted the corpus to be the only thing that changed between them. 3,000 steps draws 96,000 passages in batches of 32, which is about 23 passes over the starter run's 4,132 training passages and about 16 over the extended run's 6,067, and for text built from repeated sentence frames I thought that was plenty. The optional third run changed only this setting, to 6,000, for a reason that comes out of the second run's results.
...
Before the corpus-extension run, which I ran after removing the 58 lines, I expected coverage to stay at 36 scorable cases, since every prompt and choice word in my four categories was still in the files. I expected opposites to drop to 0 or 1 of 3, since the three tested pairs now appear only in contrasts, reference to slip to 2 of 3 without the lines that reused the pencil, leo, and nina clauses, grammar to stay at 3 of 3, negation to stay around 1 of 3, and the 16 starter patterns and 8 new phrasings to hold at 16 and 8, as in the first version, for around 30 of 48.

Before the optional third run I expected the extra steps to turn the shortcuts the probes had found into the copy pattern, the way they did in my first version, so that ava and the door would flip to right and everything that had passed would keep passing, for 36 of 48. The door was the case I was least sure of, since the cleaned files only ever correct closed to open.
...
The 24 extension tests cover eight skills the classroom sentences never touch, and in the starter run every one of them was unscorable, since the runner refuses to score a case whose prompt or answer choices include a word outside the vocabulary. I chose four skills to teach, grammar, opposites, negation, and reference, because they are teachable with short sentences and because they sit at different distances from what the starter corpus already does. Grammar is agreement, a local pattern between neighboring words, which is the kind of thing a next-word model picks up first. Opposites is a word association like the starter's own domain associations. My file states 21 pairs of opposites outright, like `the opposite of big is small .`, and the three pairs the tests ask about, hot and cold, empty and full, and noisy and quiet, only ever appear in contrasts like `the cup was cold but the pillow was hot .`, which is the kind of teaching the course's eval guide suggests for opposites. One of the other pairs does put a test's answer in an answer frame, since `the opposite of loud is quiet .` is in the file and quiet is the answer to the noisy test, with loud as one of its wrong choices, and I left that pair in because it states a different pair. Negation and reference need the model to reach back across a sentence boundary and copy a specific earlier word, which is the thing attention is for, and reference also needs it to tell the two people in a story apart. I left sequence, spatial relations, everyday knowledge, and categories alone, so their 12 cases stay unscorable in every run and count as zero in the all-case rate.
...
| --- | --- | --- | --- |
| [corpus/grammar.txt](corpus/grammar.txt) | 812 | Singular and plural subjects with is, are, was, were, and am, and yesterday, today, and every day with past, present, and continuous verb forms | `one duck is wet .` and `yesterday the nurse waited at the store .` |
| [corpus/opposites.txt](corpus/opposites.txt) | 312 | 21 pairs of opposites in four short frames, and all 24 pairs, including the three tested ones, in four sentence frames, each pair written in both directions | `the opposite of big is small .` and `the cup was cold but the pillow was hot .` |
| [corpus/negation.txt](corpus/negation.txt) | 523 | Three-sentence corrections about objects and colors or states, and about people and foods | `the bike was not brown.it was green.the bike was green.` |
| [corpus/reference.txt](corpus/reference.txt) | 503 | Two-sentence stories in which one person gives, lends, hands, sends, calls, phones, or writes to another, and the second sentence says who thanked, answered, received, or gave | `emma gave rosa a hat.rosa thanked emma.` |

The stories in the last two files are written with no space after the periods inside them, and that is deliberate. The notebook splits imported text into passages wherever a period is followed by whitespace, so a three-sentence story written normally would become three one-sentence passages and the model would never see a period followed by more words, let alone learn to copy a word from before one. Written this way each story stays one passage, and the tokenizer still separates the periods, so the model sees `the bike was not brown . it was green . the bike was green .` as one sequence of 15 tokens plus the start and end tokens, the same shape as the test prompts. `corpus_manifest.json` shows all 523 negation lines and 503 reference lines arriving as exactly that many passages.

No test prompt or test story appears in the material. The notebook itself withholds classroom sentences containing a reserved prefix, rejects any imported file containing an exact test prompt, and checks every final passage again, and the generator adds stricter rules on top. It refuses any line containing a test prompt or the first sentence of a test story, it never puts the three test couples together, so maya never appears with leo, ella with finn, or omar with nina, and it never puts a test subject with either of its test items, so ava never appears with tea or milk, the box never with red or blue, and the door never with open or closed. It also drops three kinds of near-copy. No line states a tested opposite pair in an answer frame, no correction goes in the direction a test uses, so nothing is corrected from red to blue, open to closed, or tea to milk, and nothing between the two words of a tested opposite pair, and no line reuses a clause from a test story, like `the person who received the pencil was` or `she bought milk`. Every one of the test words still appears in other lines, since the tests need them in the vocabulary, and the generator rereads the four files and checks every rule each time it writes them. What these rules cannot judge is how close the rest of the material sits to the tests, which I lay out under Keeping the tests out of training.

Adding the files took the corpus to 6,742 unique passages, 4,592 from the classroom and 2,150 from the files, split 6,067 to 675, and the vocabulary to 424 types plus the three special tokens, 427 in all, still under the cap with 0% unknown tokens in both splits. The network grew to 130,496 parameters because the tied embedding table has more rows. The manifest and vocabulary report are [results/expanded/corpus_manifest.json](results/expanded/corpus_manifest.json) and [results/expanded/vocabulary_report.json](results/expanded/vocabulary_report.json).
...
Same fixed panels of 20 documents each, drawn from this run's split, so these numbers are not comparable to the starter run's, and the brief says as much about different corpora. The starting loss is again about the log of the vocabulary size, which is 6.06 for 427 words. The validation panel ended below the training panel, which with 20 passages in each says more about which passages were drawn than about the model. The table is [results/expanded/history.json](results/expanded/history.json) and the plot is [results/expanded/training_curves.svg](results/expanded/training_curves.svg).

![Loss on the fixed training and validation panels during the corpus-extension run](results/expanded/training_curves.svg)

The samples, from [results/expanded/samples/](results/expanded/samples/), show the one thing I'd point to in this run. At both checkpoints the first line is a reference story whose second sentence names theo as the person who received the key, when the first sentence gave it to rosa, and the optional run's samples, drawn with the same seed, name rosa at both 3,000 and 6,000 steps. These are draws at temperature 0.8, so a single line says what the model happened to produce and only the probabilities say what it prefers, which is what the probes below are for. The third line at step 3,000, `nora gave sara a lamp . sara thanked nora .`, is a thanks story with the names the right way around.
...
step 1500
luca handed a key to rosa . the person who received the key was theo .
the new mango was mentioned in the fruit report yesterday .
the team discussed the deposit and the interest at the bank .
the new bus was mentioned in the travel report yesterday .

step 3000
luca handed a key to rosa . the person who received the key was theo .
we learned about the important buyer during a discussion of support .
nora gave sara a lamp . sara thanked nora .
the different customer was mentioned in the purchase report yesterday .
```

On the 48 tests the untrained model got 7 of 48 with 36 scorable, and the trained model got 34 of 48, all 16 starter patterns, all 8 new phrasings, and 10 of the 12 newly scorable extension cases. Grammar, opposites, and reference were 3 of 3 each and negation was 1 of 3. Against my prediction of around 30, opposites and reference both held at 3 of 3 where I expected them to drop, and the other categories came out as I expected. Opposites held on much less probability than in my first version, with cold at 0.093 against warm at 0.019 for the hot case, where the first version had cold at 0.451. For the hot and empty cases, the contrasts plus the answer frame the other 21 pairs teach were enough to put the right word first among the four choices. The noisy case also had `the opposite of loud is quiet .`, and it kept the most of its probability, 0.179 against 0.399 in the first version. I think the pair statements I removed were most of the first version's confidence, though the split changed along with them. The new phrasings went from 4 of 8 in the starter run to 8 of 8. Two things changed between those runs at once, the training data and, because the vocabulary is a different size, the network's random starting weights, so I can't say from these two runs alone that the new material caused that gain. One plausible mechanism is that the classroom sentences only ever put yesterday at the end of a sentence while my grammar file puts it at the start in 36 of the 200 lines that use it, and two of the four phrasings the starter model missed begin with yesterday, but I haven't tested that.

The two negation misses are the concrete failures in this run. For `ava did not buy tea . she bought milk . ava bought` it chose rice at 0.068, over tea at 0.045 and milk at 0.039. For `the door is not open . it is closed . the door is` it chose open, the word the story rejected, at 0.433 against 0.007 for closed. It got the box right, with blue at 0.563. The door is where the cleanup visibly changed the data, since my first version corrected open to closed on a bike, a book, and a chair, and got the door right, while the cleaned files only ever correct closed to open. The probes below show that this model answered open for every door and gate story, whichever way the story went.

### The optional 6,000-step run

I first proposed doubling the training steps as my next experiment after my first version's 3,000-step model missed two negation cases and, in the probes, answered several flipped stories with a favorite word or name, and I reran it on the cleaned corpus, keeping the corpus, the learning rate, the seed, and the split exactly the same as the corpus-extension run. I chose that budget after seeing 3,000-step evals and probes, so its scores are not independent evidence that more training helps. It's archived at [results/expanded-6000-steps/](results/expanded-6000-steps/) and it is extra evidence, not one of the two required experiments.
...
The panels are the same 20 and 20 documents as the corpus-extension run, so these are comparable to that run's table, with the caveat that the cosine decay was spread over twice the steps, which is why the step-3,000 row here is not the same as the step-3,000 row above. Neither panel loss improved after step 3,000, and the training panel ended slightly higher. The trained model got 33 of 48. Every one of the 12 extension cases I taught for was right, with blue at 0.971 for the box, milk at 0.685 for ava, and closed at 0.056 against open at 0.013 for the door, but 3 of the 8 new phrasings of the starter patterns went wrong, so the total fell by one. My prediction for this run was half right. The negation misses flipped as I expected, including the door I was least sure of, but I expected everything that had passed to keep passing, and the new phrasings did not.

The three phrasings it lost are `the bank report discussed the bond and the`, `yesterday our office discussed the platform and the`, and `the store report discussed the subscriber and the`, and in all three the four choice words ended up with almost no probability. Return was at 0.001 behind code at 0.003, update at 0.0036 behind treatment at 0.0042, and support at 0.0002 behind health at 0.0005, so the pick came down to tiny margins. The educator phrasing stayed right, but student fell from 0.185 at 3,000 steps to 0.003. My first version's 6,000-step run also lowered six of the eight answer probabilities for these phrasings and kept all eight right, so what the two runs share is the falling probability on the new wordings, and which ones flip looks like chance at these margins. I read the falling probabilities as the longer run fitting the model more tightly to the exact classroom frames, which is the brief's warning that held-out sentences share templates with training, but it's one run of each on one seed, and I did not run a version that separates the extra steps from the stretched schedule.
...
| Expanded corpus, optional run | Trained, 6,000 steps | 33 | 36 | 91.7% | 16 of 16 | 5 of 8 | 12 of 24, 12 scorable | [results/expanded-6000-steps/language_evals/final/](results/expanded-6000-steps/language_evals/final/) |
...
| New wording | 8 | 3 of 8 | 4 of 8 | 4 of 8 | 8 of 8 | 5 of 8 |
...
Within the scorable cases, the gains come from training on patterns, and the probes below say how much of that is the pattern itself. The starter patterns went from 6 to 16 in the first experiment on the classroom sentences alone, and the 12 new extension cases went from 0 to 10 at 3,000 steps and to 12 at 6,000 steps on my files, while the optional run gave back 3 of the 8 new phrasings. The eval guide's warning that a bigger vocabulary can move the accuracy among scorable cases in either direction applies here in a mild way, since 34 of 36 is a different denominator from 20 of 24, which is why I've kept both numbers in the table.
...
| --- | --- | --- | --- | --- | --- | --- |
| lang_25, grammar | one bird | is | is, right | is 0.329, are 0.000, were 0.000, am 0.000 | was calm yesterday . | is, right |
| lang_26, grammar | the dogs | are | are, right | is 0.000, am 0.000, was 0.001, are 0.635 | are calm . | are, right |
| lang_27, grammar | yesterday she | walked | walked, right | walking 0.001, walk 0.000, walked 0.016, walks 0.002 | fast at the park . | walked, right |
| lang_28, opposites | the opposite of hot is | cold | cold, right | fast 0.001, cold 0.093, warm 0.019, heavy 0.009 | cold . | cold, right |
| lang_29, opposites | the opposite of empty is | full | full, right | full 0.037, quiet 0.004, early 0.004, soft 0.006 | us understand the important offering . | full, right |
| lang_30, opposites | the opposite of noisy is | quiet | quiet, right | loud 0.067, round 0.002, late 0.044, quiet 0.179 | open . | quiet, right |
| lang_31, negation | the box is not red . it is blue . the box is | blue | blue, right | green 0.018, yellow 0.018, blue 0.563, red 0.022 | pink . | blue, right |
| lang_32, negation | ava did not buy tea . she bought milk . ava bought | milk | rice, wrong | rice 0.068, milk 0.039, tea 0.045, bread 0.015 | honey . | milk, right |
| lang_33, negation | the door is not open . it is closed . the door is | closed | open, wrong | closed 0.007, open 0.433, wide 0.000, missing 0.001 | heavy . | closed, right |
| lang_34, reference | maya lent a book to leo . leo thanked | maya | maya, right | leo 0.008, nora 0.000, omar 0.000, maya 0.726 | maya . | maya, right |
| lang_35, reference | ella gave finn a pencil . the person who received the pencil was | finn | finn, right | sara 0.007, noah 0.008, finn 0.222, ella 0.093 | finn . | finn, right |
| lang_36, reference | omar called nina . nina answered the call from | omar | omar, right | emma 0.000, omar 0.682, nina 0.001, luca 0.016 | omar . | omar, right |
| lang_37, sequence | first wash the cup . then dry it . the last action is | dry | unscorable | unknown words action, fill, first, wash | data was | unscorable |
| lang_38, sequence | lunch happens after breakfast . the earlier meal is | breakfast | unscorable | unknown words breakfast, dinner, earlier, happens, lunch, meal, supper | kitchen . | unscorable |
| lang_39, sequence | the train arrived before the bus . the vehicle that arrived later was the | bus | unscorable | unknown words arrived, before, later, that, vehicle | about . | unscorable |
| lang_40, spatial relations | the book is inside the bag . the bag contains the | book | unscorable | unknown words contains, desk, inside, shelf | book was gray . | unscorable |
| lang_41, spatial relations | the lamp is above the desk . the desk is | below | unscorable | unknown words above, below, beside, desk, inside | not wet . | unscorable |
| lang_42, spatial relations | the ball is left of the box . the box is to the | right | unscorable | unknown words left, north, right, south | station . | unscorable |
| lang_43, everyday knowledge | water freezes into | ice | unscorable | unknown words freezes, ice, into, sand, steam, wood | the bring is the person who gave the gift was opposites . | unscorable |
| lang_44, everyday knowledge | a person uses an umbrella to stay | dry | unscorable | unknown words an, stay, umbrella, uses | the new shopper . | unscorable |
| lang_45, everyday knowledge | to see in a dark room we turn on a | light | unscorable | unknown words see, shoe, spoon, turn | discussion of care . | unscorable |
| lang_46, categories and analogies | a robin is a bird . a salmon is a | fish | unscorable | unknown words robin, salmon, tool, tree | review . | unscorable |
| lang_47, categories and analogies | a puppy grows into a dog . a kitten grows into a | cat | unscorable | unknown words grows, into, kitten, puppy | card is learning . | unscorable |
| lang_48, categories and analogies | a carrot is a vegetable . an apple is a | fruit | unscorable | unknown words an, carrot, fabric, metal, vegetable, vehicle | price . | unscorable |

The continuations for the unscorable cases are worth a look on their own. The model receives those prompts with their unknown words replaced by the `<UNK>` token, and what it writes back is a fragment of whatever frame the known words suggest, `book was gray .` after the bag prompt because my negation file taught it that objects have colors, and `the bring is the person who gave the gift was opposites .` after the water prompt, which strings a piece of my reference frame onto a word from my opposites file with nothing to anchor either one.

### What the scores do and do not show

A right answer on a four-choice test is a narrow thing, so after training I wrote extra prompts of my own, seventeen in [scripts/probe_model.py](scripts/probe_model.py), twelve at first and five more after a review pass over this README, to ask what the model had actually learned. They are not part of the suite, they never trained anything, none of their stories appears in the teaching files, and I wrote them after seeing my first version's results, so I treat them as diagnostics. The results are [results/expanded/probes.json](results/expanded/probes.json) for the 3,000-step model and [results/expanded-6000-steps/probes.json](results/expanded-6000-steps/probes.json) for the 6,000-step one.

At 3,000 steps the colors followed the story. Given `the cup is not white . it is black . the cup is`, the model put black first at 0.733, and with the story flipped it put white first at 0.679. Doors, gates, foods, and names did not follow the story. The model answered open for every door and gate story, including `the gate is not open . it is closed . the gate is`, where it put open at 0.413 against 0.010 for closed, and it answered rice for what nora bought whether the story said rice or bread, at 0.066 both times. In the gave and received stories it picked a favorite name. Given `nora gave omar a coin`, it answered omar whether the question was who gave, at 0.418, or who received, at 0.444, and it answered nora for both orders of a nora and sara pencil story, at 0.483 and 0.458. After thanked it did pick the first name in the one story I flipped, leo at 0.783. In all, 12 of the 17 probes passed, and the passes were the three grammar probes, whose prompts all start lines in my grammar file word for word, the two color copies, the object copy, the thanked story, and five that a favorite word or a favorite name would also get.

At 6,000 steps 15 of the 17 probes pass and most answers move with the story. The flipped cups get black at 0.966 and white at 0.972, nora's purchase follows the story at 0.709 for bread and 0.413 for rice, the giver and receiver probes get nora at 0.782 and omar at 0.939, and the nora and sara story now gets sara at 0.939 when she received the pencil and nora at 0.251 when nora did. The two that still fail are the gate story, where open at 0.131 still beats closed at 0.075 even though the door eval flipped to closed, and the omar and nora coin story, where omar at 0.554 beats nora at 0.106 as the receiver. That is mostly the behavior the negation and reference tests are meant to measure, and it appeared in the one run with twice the steps and the stretched schedule. So I'd read the corpus-extension run's 34 of 48 as vocabulary coverage plus a mix of real patterns and shortcuts, and the optional run's 33 of 48 as the same coverage with most of the shortcuts replaced by the patterns and three of the new phrasings lost, at least for the frames I wrote.
...
The 48 prompts, their choices, the answer key, and every eval output live in `evals/` and `results/`, and only `corpus/` is a training input. The notebook rejects a corpus folder that includes `evals/` or the project root, withholds every generated classroom sentence containing one of the 16 reserved prefixes before the split and before the vocabulary is built, rejects any imported file containing an exact test prompt, and checks every final passage once more. [results/starter/eval_separation.json](results/starter/eval_separation.json) and [results/expanded/eval_separation.json](results/expanded/eval_separation.json) record 160 withheld passages covering all 16 reserved cases in each run, along with the suite's hash. My generator's rules, described above, cover the extension cases, and the chat transcripts were written after training and never entered a corpus. All of these checks match text, so they catch exact prompts, the first sentence of each test story, and the near-copies I listed, and nothing looser.

The first version of my files, committed on 2026-09-21, did not have the near-copy rule. Its opposites file stated the three tested pairs outright in 21 lines, like `hot is the opposite of cold .` and `the opposite of cold is hot .`, its negation file had 18 corrections that used a tested pair, like `this cup is not red.it is blue.this cup is blue.`, and 19 lines reused a clause from a test story, like `eli handed a pencil to finn.the person who received the pencil was finn.`. The eval guide's advice for opposites is to "teach the vocabulary in sentences rather than pasting answer lists," and those lines read like an answer list, so I removed all 58 and reran both expanded runs before submitting. The first version is still in the repository's history at commit `01b6c8b`, with [its README](https://github.com/IsaacAVazquez/Fundamentals-of-Agentic-AI/blob/01b6c8b/assignment-03/README.md), [its opposites file](https://github.com/IsaacAVazquez/Fundamentals-of-Agentic-AI/blob/01b6c8b/assignment-03/corpus/opposites.txt), and [its trained model's eval summary](https://github.com/IsaacAVazquez/Fundamentals-of-Agentic-AI/blob/01b6c8b/assignment-03/results/expanded/language_evals/final/eval_summary.json). It scored 34 of 48 at 3,000 steps and 36 of 48 at 6,000, against 34 and 33 now. At 3,000 steps the category scores came out the same but the probabilities did not, cold at 0.451 then and 0.093 now for the hot case, and the door went from right to wrong while the box went from wrong to right. The two versions start from identical weights, since the vocabulary is the same 424 words and the untrained models hash to the same value, but the missing lines also changed the split and the order of the training batches, so this is not a clean test of what those lines did on their own.

Past those rules, my files still use several of the tests' frames word for word, like `X is not A.it is B.X is B.`, `X gave Y a Z.the person who received the Z was Y.`, and `X called Y.Y answered the call from X.`, filled with other people, objects, and colors, and some answer words still appear in those frames in other stories, like `the door is not purple.it is blue.the door is blue.` and `omar called emma.emma answered the call from omar.`, along with the loud and quiet lines above. I think that stays within the line the course draws. Its example of a supported pattern is the surgeon test, whose exact prefix is withheld while other sentences teach the association, and the classroom corpus itself keeps five to eight sentences one word away from each reserved test with its answer, like `the report about the dentist explains the patient in detail .` beside the surgeon test.
...
```
0.056, -0.133, 0.042, -0.025, -0.030, -0.126, 0.135, 0.075, 0.027, 0.034, 0.045, 0.045, -0.029, -0.145, 0.053, 0.088,
0.016, -0.070, 0.136, 0.039, -0.070, -0.039, 0.093, 0.050, 0.039, -0.046, 0.061, -0.033, 0.074, -0.117, -0.040, 0.027,
-0.051, -0.051, 0.140, -0.030, -0.064, 0.072, -0.034, -0.009, 0.090, -0.090, -0.040, -0.061, -0.001, 0.120, -0.088, 0.074,
0.050, -0.132, -0.050, 0.152, 0.166, -0.006, 0.090, -0.040, -0.083, -0.003, -0.091, 0.040, 0.120, -0.089, -0.018, 0.038
```

rounded to three places, with full precision in `inspection.json`. The cosine similarity between the two versions is 0.33, so the row was mostly rewritten, and its length, the square root of the sum of the squares, grew from 0.18 to 0.62, which is typical of an embedding that started as small noise. In the starter run the same word is row 28 of a 136-row table, and its final row is a different set of 64 numbers again, since a different vocabulary and a different random start produce a different table.

The network is nanoGPT's, unchanged. Each input token's embedding row is added to a learned row for its position, then two transformer blocks each apply attention followed by a small feed-forward layer, with a normalization step before each and the block's input added back to its output, and the last layer's output is multiplied by the same embedding table, transposed, to get one score per vocabulary word. What makes it a neural network is that every one of those steps is a weighted sum of its inputs, the weights are the 130,496 parameters, and a nonlinearity, GELU here, sits inside the feed-forward layer so that stacking the steps does more than one big weighted sum could. The scores for the next word go through a softmax, which turns them into probabilities that sum to one, and the loss for one target token is the negative log of the probability the network gave to the token that actually came next. Averaged over all the targets in a batch, that is the number training pushes down. Before training the network had no reason to prefer any word, so its probabilities were close to uniform and the panel losses were 6.03 and 6.04, right about the natural log of 427.

A gradient is the derivative of that batch loss with respect to one parameter, and PyTorch computes one for every parameter by backpropagation. The notebook saves the first coordinate of customer's row at the first step. Its value was 0.015770, its gradient was 0.004162, and the learning rate at that step was 0.00001, since the warmup starts at a hundredth of 0.001. A positive gradient means increasing the number would raise the loss, so the optimizer moved it down, and after the update it was 0.015760. The size of the move is the learning rate, because AdamW's very first step divides the gradient by its own magnitude, which gives a step of the learning rate in the direction that lowers the loss, and its weight decay on a number this small changes the ninth decimal place. In the starter run the same coordinate also had a positive gradient, 0.000693, and moved down by the same 0.00001, from -0.057592 to -0.057602. Later steps are not this simple, because AdamW keeps running averages of the gradients and their squares and the cosine schedule shrinks the learning rate, but every one of the 3,000 updates is the same idea applied to all 130,496 parameters at once.

What that did to the predictions is the point. Before training, the most likely word after `the customer` was customer itself at 0.0045, which is about twice the uniform 0.0023 and means nothing. After training, the top six words are returned at 0.192, ordered at 0.190, selected at 0.176, reviewed at 0.170, recommended at 0.131, and compared at 0.119, which together take 98% of the probability, and those are exactly the six verbs that follow a shopper noun in the classroom corpus's `the customer ordered the product after checking the price .` frame. The starter run's trained model gives the same six verbs in a slightly different order.

Attention is the step that lets the prediction for one position use the earlier positions. For each position the block computes a query and, for every position, a key, scores the query against each key, masks out every position after the current one, and turns the scores into weights with a softmax, then adds up the values of the earlier positions with those weights. The mask is why the model can't see the future, and it's why the same network can be trained on whole passages at once, since each position is predicted from what came before it. The notebook records the weights of the first head in the first block for `the customer`. The start token attends only to itself, the attends to the start token at 0.386 and to itself at 0.614, and customer splits its attention 0.426 on the start token, 0.087 on the, and 0.487 on itself. These are one head's weights for one short prompt and they say nothing about which head does the copying in the reference stories, which I did not trace.

Generating text is sampling from those probabilities one token at a time. The notebook starts from the start token, computes the probabilities for the next word, draws one, appends it, and repeats until it draws the end token or reaches 32 tokens, and the eval runner and the chat do the same from a prompt. Temperature is a division of the scores by a number before the softmax, at inference only, and no weight changes when it changes. Below 1 it sharpens the distribution so the likely words become more likely, and above 1 it flattens it. In this run's comparison, in [results/expanded/temperature_comparison.json](results/expanded/temperature_comparison.json), all four samples at 0.3 are classroom sentences, which are the most probable frames, the four at 0.8 are the step-3,000 samples above, with two reference stories, and at 1.2 the first two lines are unchanged, the thanks story turns into `nora gave sara a lamp . sara person who received the lamp was sara .`, and the last line breaks down into `finn did not pick eggs . he picked salt . rice instead . teacher listen .`. The starter run's comparison, in [results/starter/temperature_comparison.json](results/starter/temperature_comparison.json), moves less, with two of four lines changing at 0.3 and nothing at all changing at 1.2, because a 136-word model trained on nine frames is so confident that flattening the scores a little doesn't change the draws.
...
Model: the opposite discussed .
Unknown words: ?, capital, france, what
```

The first, third, and fourth replies are what the corpus taught, including the cup story, which my first version's model had answered with the rejected color. The second is a failure. Asked for the opposite of cold, a pair that now appears in my files only in contrasts, it drew pear, and since that is one draw at temperature 0.8, it only shows what the model happened to produce. The fifth is the limitation the assignment asks to show. The question's words are mostly outside the vocabulary, the model receives them as unknown tokens, and it replies with `the opposite discussed .`, words from my files with no meaning together, because continuing text is the only thing it does and it has no idea of a question. The notebook's own chat cell, section 10, produced one more turn from the same model during Run All, `the customer` followed by `returned the item after checking the price .`, saved in [results/expanded/chat_transcript.json](results/expanded/chat_transcript.json).

I also ran a shorter session against the optional 6,000-step model, in [results/chat/chat_transcript_6000_steps.json](results/chat/chat_transcript_6000_steps.json), with its log beside it and [a drawing of that log](results/chat/chat_session_6000_steps.png) made by [scripts/render_session.py](scripts/render_session.py), since only the main session has a screenshot. It answered the same cup story with black, answered `nora gave omar a coin . the person who gave the coin was` with nora, continued `the customer` with `reviewed the item after checking the price .`, and, given a sentence that mixes words from three of the classroom domains, `the surgeon and the mango walked to the bank`, replied `yesterday .`, one draw that ends the sentence the way many lines in my grammar file end. Both sessions used the interface's default temperature of 0.8 and 24-token limit, and neither prompt was ever longer than the context.

## One limitation and the next experiment

The limitation I'd point to is that a passing score on these tests can come from a shortcut. At 3,000 steps the model answered all three reference tests correctly, and the probes showed that in the gave and received stories it picked the same name whichever question I asked, so it had not learned which person gave and which received. The door test is the other face of it. It turned on which word the model preferred, closed in my first version, whose files corrected open to closed, and open now, when my files only correct closed to open, and neither version was reading the story. The multiple-choice score can't tell those apart from the real pattern, and neither can a fluent sample, which is why I'd trust the flipped-story probes more than either. In the one run with twice the steps and the stretched schedule, most of that went away for the seventeen probes I wrote while three of the new phrasings flipped, and I haven't tested frames I didn't write.

The next experiment I'd run, beyond the one I already did, is the remaining four extension skills with the same method, at both 3,000 and 6,000 steps, since the longer run removed shortcuts and lowered the new phrasings' probabilities here. Sequence, spatial relations, everyday knowledge, and categories would each get a generated file with the test words in different sentences, which would take coverage to 48 of 48, and this time I'd write the flipped-story probes before training. My prediction is that everyday knowledge and categories pass, since they are associations of the kind the opposites file taught, that sequence and spatial relations are the new negation, right on the exact frames and wrong on the flipped ones at first, and that the probes will say more than the score does.
...
| [results/chat/](results/chat/) | The chat transcripts and session logs, the Terminal screenshot, and the drawing of the 6,000-step session |
