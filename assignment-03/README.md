...
To run it again locally, clone the repository and set up Python 3.13 in this folder. I used `scripts/setup.sh`, which builds `.venv` with uv from Homebrew's Python 3.13 and installs `requirements.txt` plus NumPy, pip, and the Jupyter pieces a scripted run needs. NumPy is there because the course's `run_evals.py` hashes model weights through NumPy and the PyTorch wheel does not install it. The course's own instructions use a plain virtual environment and pip instead, which also works as long as NumPy is added, and I checked that `python3.13 -m venv .venv` followed by `.venv/bin/pip install -r requirements.txt numpy` runs the notebook, the evals, and the chat with the same results. Either way the notebook's setup cell installs the PDF reader if it's missing.
...
The 48 prompts, their choices, the answer key, and every eval output live in `evals/` and `results/`, and only `corpus/` is a training input. The notebook rejects a corpus folder that includes `evals/` or the project root, withholds every generated classroom sentence containing one of the 16 reserved prefixes before the split and before the vocabulary is built, rejects any imported file containing an exact test prompt, and checks every final passage once more. [results/starter/eval_separation.json](results/starter/eval_separation.json) and [results/expanded/eval_separation.json](results/expanded/eval_separation.json) record 160 withheld passages covering all 16 reserved cases in each run, along with the suite's hash. My generator's four rules, described above, cover the extension cases the same way, and the chat transcripts were written after training and never entered a corpus. All of these checks match normalized text, so they catch exact prompts and the exact story sentences and nothing looser. Past that, my files use the tests' frames word for word with different names and objects, and they often keep the answer words. The opposites file states the three tested pairs directly in 21 of its 333 lines, such as `hot is the opposite of cold .` and `the opposite of cold is hot .`, and uses them in 18 more contrasts like `the cup was cold but the pillow was hot .`. The negation file has the box test's red-to-blue correction on a cup, a bike, and a road, as in `this cup is not red.it is blue.this cup is blue.`, and the door test's open-to-closed correction on a bike, a book, and a chair. I think that stays within the line the course draws for its own starter tests. The brief's example of a supported pattern is the surgeon test, whose exact prefix is withheld while other sentences teach the association, and the classroom corpus keeps five to eight sentences one word away from each reserved test with its answer, like `the report about the dentist explains the patient in detail .` beside the surgeon test. The opposites lines are the closest call, since the eval guide's advice for opposites is to "teach the vocabulary in sentences rather than pasting answer lists."
