...
The tests themselves live in `evals/`, and the only training input is `corpus/`. The notebook refuses to import a file that contains a test prompt, and my generator refuses to write one, so no test question reaches training word for word. That is the one rule, since the instructor clarified in class on 2026-09-22 that the eval questions themselves are what must stay out and that stricter guardrails should come out so relevant material stays in. So the teaching files use the tests' words, word pairs, and sentence frames on purpose, mostly with other names and objects, like `the opposite of cold is hot`, which the README lays out under Keeping the tests out of training.
...
| `scripts/make_teaching_corpus.py` | Generates the four teaching files from word lists and sentence frames, drops any line that contains a test prompt, and rechecks the written files |
...
The README covers what actually happened, including the two stricter versions of my teaching files before the instructor's clarification, the one reference test the corpus-extension model got wrong, the probes that showed which passes came from shortcuts, and the optional run with twice the training steps and a stretched schedule, where the shortcuts went away on my seventeen probes and one of the new phrasings of the starter patterns flipped. This file stops at what the code does.
